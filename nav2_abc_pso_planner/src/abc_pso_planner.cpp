#include "nav2_abc_pso_planner/abc_pso_planner.hpp"  // Header del planner ABC+PSO
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(logger_, "Configured ABCPSOPlanner with num_waypoints=%d, w=%.2f, wdamp=%.2f, c1=%.2f, c2=%.2f, swarm_size=%d, max_iterations=%d, w_length=%.2f, w_offmap=%.2f, w_cell=%.2f, w_unknown=%.2f, w_lethal=%.2f, w_inflated=%.2f",
              num_waypoints_, w_, wdamp_, c1_, c2_, swarm_size_, max_iterations_, w_length_, w_offmap_, w_cell_, w_unknown_, w_lethal_, w_inflated_);
}
...
// Cachea la mejor ruta para evitar recomputaciones innecesarias
...
{
  // Si cambia el goal, reiniciamos la caché
  ...
  // Comparación con caché
  if (std::isfinite(cached_best_cost_) && best_cost >= cached_best_cost_ &&
      !cached_path_.poses.empty()) {
    // Reusar camino cacheado comenzando desde el punto más cercano al inicio
    size_t closest_idx = 0;
    double best_dist = std::numeric_limits<double>::infinity();
    for (size_t i = 0; i < cached_path_.poses.size(); ++i) {
      double dx = cached_path_.poses[i].pose.position.x - start.pose.position.x;
      double dy = cached_path_.poses[i].pose.position.y - start.pose.position.y;
    ...
    nav_msgs::msg::Path reused;
    reused.header = cached_path_.header;
    geometry_msgs::msg::PoseStamped s = start;
    s.header = reused.header;
    if (closest_idx < cached_path_.poses.size()) {
      size_t next_idx = std::min(closest_idx, cached_path_.poses.size() - 1);
      double dx = cached_path_.poses[next_idx].pose.position.x - start.pose.position.x;
      double dy = cached_path_.poses[next_idx].pose.position.y - start.pose.position.y;
      double yaw = std::atan2(dy, dx);
      s.pose.orientation.z = std::sin(yaw * 0.5);
      s.pose.orientation.w = std::cos(yaw * 0.5);
    } else {
      s.pose.orientation.w = 1.0;
    }
    reused.poses.push_back(s);
    for (size_t i = closest_idx; i < cached_path_.poses.size(); ++i) {
      reused.poses.push_back(cached_path_.poses[i]);
    }
    RCLCPP_INFO(logger_, "Reutilizando ruta cacheada");
    return reused;
  }

  // Actualiza caché con nueva ruta
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  std::mt19937 gen(std::random_device{}());
  std::uniform_real_distribution<> jitter(-0.3, 0.3);  // Ruido posición
  ...
  for (int i = 0; i < swarm_size_; ++i) {
    ...
    for (int j = 0; j < num_waypoints; ++j) {
      double t = static_cast<double>(j + 1) / (num_waypoints + 1);
      double xi = x0 + t * (x1 - x0) + jitter(gen);  // X con jitter
      double yi = y0 + t * (y1 - y0) + jitter(gen);  // Y con jitter
